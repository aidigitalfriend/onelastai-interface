...
const ChatBox: React.FC<ChatBoxProps> = ({ 
  ...
  const removePendingFile = (fileId: string) => {
    // We need to handle this through the parent, but for now just call clearAll if only one file
    if (onClearPendingFiles && pendingFiles.length <= 1) {
      onClearPendingFiles();
    }
  };
    ...
    <main className="relative flex-grow bg-black/20 flex flex-col overflow-hidden z-10 m-2 sm:m-4 rounded-lg shadow-[0_0_40px_rgba(0,0,0,0.6)] border border-gray-800/40 backdrop-blur-md">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {msg.files.map((file) => (
                        <div 
                          key={file.id} 
                          className="flex items-center gap-2 bg-black/40 border border-gray-700 rounded-lg px-3 py-2 hover:border-cyan-500/50 transition-colors cursor-pointer"
                          onClick={() => {
                            if (file.dataUrl && file.type.startsWith('image/')) {
                              ...
                              const isCode = file.type.includes('javascript') || file.type.includes('typescript') || file.type.includes('json') || file.type.includes('xml') || file.type.includes('html') || file.type.includes('css') || file.name.match(/\.(js|ts|jsx|tsx|json|xml|html|css|py|java|c|cpp|h|go|rs|rb|php|swift|kt)$/);
      ...
      <div className="relative z-40 p-4 border-t border-gray-800/50 bg-[#0a0a0a]/90 backdrop-blur-xl flex flex-col gap-2">
        ...
        {pendingFiles.length > 0 && (
          <div className="flex flex-wrap gap-2 p-2 bg-emerald-500/10 border border-emerald-500/30 rounded-lg">
            {pendingFiles.map((file) => (
              <div key={file.id} className="flex items-center gap-2 bg-black/40 border border-gray-700 rounded px-2 py-1.5">
                {getFileIcon(file.type)}
                <div className="flex flex-col">
            ...
            <button 
              ...
              className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-red-500/10 rounded transition-all self-center"
