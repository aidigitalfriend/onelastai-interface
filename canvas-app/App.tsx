...
// AI Models - 5 Providers, 8 Models (Custom branded names)
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCredits = async () => {
      try {
        ...
        console.log('Not authenticated or credits unavailable');
      }
    };
    fetchCredits();
    ...
    if (urlParams.get('payment') === 'success') {
      ...
      if (credits) {
        setUserCredits(prev => prev + parseInt(credits, 10));
      }
      // Clean URL
  ...
  const handleGenerate = async (
    ...
    if (!user) {
      setGenState({
        isGenerating: false,
        error: '🔐 Please log in to use Canvas. Click here to login: ' + 'https://onelastai.co/auth/login?redirect=' + encodeURIComponent(window.location.href),
        progressMessage: '',
      });
      return;
    }
    ...
    if (userCredits <= 0) {
      setGenState({
        isGenerating: false,
        error: '⚠️ Insufficient credits. Please purchase credits to continue using Canvas.',
        progressMessage: '',
      });
  ...
  return (
    <div className={`flex flex-col h-screen ${isDarkMode ? 'bg-[#0a0a0a] text-gray-300 matrix-bg' : 'bg-gray-100 text-gray-800'}`}>
              ...
              {activePanel === 'settings' && (
                <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#111]/95">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
                    {/* Credits Section */}
                    <div className="p-4 bg-gradient-to-br from-cyan-500/10 to-emerald-500/10 border border-cyan-500/30 rounded-lg">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <p className="text-xs font-bold text-cyan-400 uppercase tracking-wider">AI Credits</p>
                          <p className="text-3xl font-bold text-white mt-1">{userCredits}</p>
                        </div>
                      ...
                      <button 
                        ...
                        className="w-full py-2.5 text-sm font-bold bg-gradient-to-r from-cyan-500 to-emerald-500 text-white rounded-lg hover:shadow-[0_0_20px_rgba(34,211,238,0.4)] transition-all"
                      >
                        ⚡ Buy More Credits
                      </button>
                    </div>
      ...
      <CreditPurchase 
        ...
        onCreditsPurchased={(newBalance) => setUserCredits(newBalance)}
      />
    </div>
  );
};
