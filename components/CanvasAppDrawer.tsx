...
// Determine canvas app URL based on environment
const getCanvasAppUrl = (): string => {
  // In production (maula.onelastai.co), canvas is at /canvas/
  if (typeof window !== 'undefined' && window.location.hostname === 'maula.onelastai.co') {
    return '/canvas/';
  }
  // In development, we'll try localhost ports
  return '';
};
...
const CanvasAppDrawer: React.FC<CanvasAppDrawerProps> = ({ isOpen, onClose }) => {
  ...
  // Try to find canvas-app
  useEffect(() => {
    if (!isOpen) return;
    
    const prodUrl = getCanvasAppUrl();
    
    // If in production, use the production URL directly
    if (prodUrl) {
      setCanvasAppUrl(prodUrl);
      setIsLoading(false);
      return;
    }
    
    // In development, try to find canvas-app on various ports
